...
// 懒汉模式(单例模式) 局部静态变量法（这种方法不需要加锁和解锁操作）
Log* Log::Instance() {
    static Log log;
    return &log;
}
...
void Log::FlushLogThread() {
    ...
    //Log::Instance()返回的是类实例的指针，所以可以在其后加‘->’来调用该实例的成员函数
}
...
void Log::AsyncWrite_() {
    ...
    while(deque_->pop(str)) {
        lock_guard<mutex> locker(mtx_);
        fputs(str.c_str(), fp_);    //fputs不会自动换行
    }
}
...
void Log::init(int level, const char* path, const char* suffix, int maxQueCapacity) {
    ...
    struct tm *sysTime = localtime(&timer);//将时间戳转化为本地时间
    struct tm t = *sysTime;
    path_ = path;   //!为什么要再次赋值
    suffix_ = suffix;   //!为什么要再次赋值
